...
contract StakingTest is Test{
    ...
    function test_stake(address _recommender, address _user, uint256 _amount) internal {
        vm.startPrank(_user);
        deal(USDT, _user, _amount);
        IERC20(USDT).approve(address(staking), _amount);
        staking.referral(_recommender);
        staking.stake(_amount);
        vm.stopPrank();
    }
    //测试质押收益和邀请奖励收益
    function test_referralAward() public {
        test_stake(initialCode, user, 100000e18);
        //升级到V1前没有任何收益
        address user1 = address(5);
        address user2 = address(6);
        address user3 = address(7);
        test_stake(user, user1, 4000e18);
        test_stake(user, user2, 4000e18);
        test_stake(user, user3, 2000e18);
        ...
        (,,,,uint256 totalAward,) = staking.getUserInfoBasic(user);
        // console.log("totalAward:",totalAward);
        assertEq(totalAward, 100e18);
        (uint256 referralNum,
        uint256 performance,
        uint256 referralAward,
        uint256 subCoinQuota,) = staking.getUserInfoReferral(user);
        assertEq(referralNum, 4);
        assertEq(performance, 11000e18);
        assertEq(referralAward, 100e18);
        ...
        (,,,,uint256 totalAward0,) = staking.getUserInfoBasic(user);
        assertEq(totalAward0, 300000e18);
