...
contract Staking is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard{
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        // 转入质押代币
        TransferHelper.safeTransferFrom(flap, msg.sender, address(this), _amount);
    ...
    /// @notice 赎回 flap（会把 pending + 本次应得累积到 pending，然后按 10% 手续费销毁质押份额）
    function unstake(uint256 _amount) external nonReentrant {
        require(_amount > 0, "zero amount");
        User storage user = userInfo[msg.sender];
        require(user.stakingAmount >= _amount, "insufficient stake");
        ...
        TransferHelper.safeTransfer(flap, DEAD, fee);
        TransferHelper.safeTransfer(flap, msg.sender, _amount -fee);
